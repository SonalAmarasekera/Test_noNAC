...
class HParams:
    ...
    train_root: str = "/workspace/2speakers/wav16k/min/tr"
    val_root:   str = "/workspace/2speakers/wav16k/min/cv"
...
def main():
    ...
    for epoch in range(hp.epochs):
        ...
        for step_in_epoch, batch in enumerate(prog):
            ...
            # uPIT SI-SDR loss
            loss, idx = pit_sisdr(yhat1, yhat2, wav_s1, wav_s2)
            avg_sisdr_db = (-loss).item() / 2.0  # per-speaker average dB
        ...
        print(f"[epoch: {epoch+1}] train_sisdr_loss: {epoch_train_loss:.4f} avg_sisdr: {avg_sisdr_db:.4f}")

        with torch.no_grad():
                # A) Complementarity & bounds
                msum = (m1_btF + m2_btF).mean().item()
                mmin = torch.minimum(m1_btF, m2_btF).mean().item()
                mmax = torch.maximum(m1_btF, m2_btF).mean().item()
                # With softmax, msum should be ~1.0 (exact 1.0 up to numerical errors).
                print(f"[mask] mean(m1+m2)≈{msum:.4f} | mean(min)≈{mmin:.4f} | mean(max)≈{mmax:.4f}")

                # B) Mask entropy (are masks becoming confident?)
                m = torch.stack([m1_btF, m2_btF], dim=-1)             # [B,T,F,2]
                ent = -(m.clamp_min(1e-9) * (m.clamp_min(1e-9).log())).sum(-1).mean().item()
                print(f"[mask] entropy≈{ent:.4f} (lower means more confident)")

                # C) Mixture consistency in STFT domain
                # Use the *same* X_bFT that went into masks (no extra STFT call)
                resid = ( (Y1_bFT + Y2_bFT) - X_bFT ).abs().mean().item()
                print(f"[mix-consistency] |Y1+Y2 - X|_mean≈{resid:.6e}")

                # D) Standard benchmark testing
                mix = Y1_bFT + Y2_bFT
                base = 0.5 * (sisdr(mix, Y1_bFT).mean() + sisdr(mix, Y2_bFT).mean())
                sisdr_i = avg_sisdr_db - base.item()
                print(f"Standard benchmark SI-SDR: {sisdr_i:.4f}")

                #E) Reorder using PIT permutations
                # idx == 0 means (yhat1->y1, yhat2->y2); idx == 1 means swap
                mask = (idx == 0).unsqueeze(1)  # [B,1]
                yhat1_ordered = torch.where(mask, yhat1, yhat2)
                yhat2_ordered = torch.where(mask, yhat2, yhat1)
